...
def get_case_id():
    ...
    if len(sys.argv) < 2:
        raise ValueError(
            ...
            "Example:\n"
            "python evaluation/scripts/evaluate_baseline_metrics.py case_001"
        )
...
def get_paths(case_id):
    ...
    result_file = (
        ...
        / "results"
        / "baseline"
        / f"{case_id}_baseline_metrics.json"
    )
...
def load_output(output_file):
    ...
    if not output_file.exists():
        raise FileNotFoundError(
            f"Baseline Gemini output not found:\n{output_file}"
        )
    ...
    if isinstance(vitals, dict):
        ...
        if heart_rate is not None:
            ...
            checks["Heart rate"] = (
                hr in text
                or f"{hr} bpm" in text
                or f"heart rate of {hr}" in text
                or f"heart rate: {hr}" in text
            )
...
def clinical_checks(output):
    ...
    checks["No confirmed diagnosis"] = not (
        detect_confirmed_diagnosis(output)
    )
    ...
    treatment_matches = (
        detect_treatment_prescription(output)
    )
...
def main():
    ...
    print(
        "- No RAG evidence injection"
    )

    print(
        "- No DeepSeek-R1 reasoning"
    )
    ...
    if unsupported:
        ...
        print(
            "\nUnsupported patient information detected:"
        )
    ...
    print(
        "\nClinical considerations reflected:"
    )

    for item in cardiac_matches:
    ...
    if treatment_matches:
        ...
        print(
            "\nNo treatment-prescription language detected."
        )
    ...
    result = {
        ...
        "evaluation_type":
            "Baseline Gemini decision-support evaluation",
        ...
        "architecture": {
            ...
            "original_literature_tool":
                True,

            "original_safety_layer":
                True,
        },
